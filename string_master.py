...
def read_master_data(path):
    master_data= pd.read_excel(path,sheet_name="deployments")
    return master_data
...
def string_search_master(search_bar,input_file_path):
    ...
    new_input_df = (input_df[["DeploymentID", "clean_MasterUseCase"]].dropna())
    line = search_bar
    text = preproc(line)
    ntext = preprocess(text)
    data = new_input_df[new_input_df["clean_MasterUseCase"].str.contains(ntext)]
