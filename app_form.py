...
def main():
    ...
    with st.form(key="similarity"):
        ...
        with col2:
            ...
            sentence = placeholder_search.text_input('Enter search string')
                            ...
                            if st.session_state.radio=="Search In Bot Description Table":
                                if st.session_state.radiobutton_search=="Semantic Search":
                                    ...
                                    if df_result.shape[0]>0:
                                            ...
                                            text_downloader("master usecase",df_result,"Bot Description_Semantic Search")
                                elif st.session_state.radiobutton_search=="String Search":
                                    df,df_result=string_search_bot(sentence.strip().lower(),"data/Deployment.xlsx","Bot Description")
                                    
                                    if df_result.shape[0]>0:
                                            msg.success('Success...')
                                            st.session_state.counter_two=df.copy() 
                                            text_downloader("master usecase",df_result,"Bot Description_String Search")
                                ...
                                if st.session_state.radiobutton_search=="Semantic Search":
                                    ...
                                    if df_result.shape[0]>0:
                                            msg.success('Success...')
                                            text_downloader("master usecase",df_result,"Master Usecase_Semantic Search")
                                    ...
                                    if df_result.shape[0]>0:
                                            msg.success('Success...')
                                            st.session_state.counter_two=df_result.copy()        
                                            text_downloader("master usecase",df_result,"Master Usecase_String Search")
